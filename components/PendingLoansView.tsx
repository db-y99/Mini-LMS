...
import { Search, Clock, AlertTriangle, CheckCircle2, XCircle, FileText, CalendarClock, Check, User, Eye, X, Calculator, Shield } from 'lucide-react';
...
export const PendingLoansView: React.FC = () => {
  ...
  const getActionButtons = (loan: PendingLoanItem) => {
    ...
    // For security role, show verify & lock button
    if (isSecurityRole && loan.status === 'pending_security') {
      return (
        <div className="flex items-center justify-end gap-2">
          <button
