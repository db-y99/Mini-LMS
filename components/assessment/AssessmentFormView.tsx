        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              />
              {assessmentForm.collateralValue > 0 && (
                <p className="text-xs text-slate-500 mt-1">{formatCurrency(assessmentForm.collateralValue)}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {assessmentForm.marketValue > 0 && (
                <p className="text-xs text-slate-500 mt-1">{formatCurrency(assessmentForm.marketValue)}</p>
              )}
            </div>
          ...
          {/* LTV Calculation */}
          {assessmentForm.collateralValue > 0 && loan && (
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="space-y-6">
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    <CheckCircle2 className={`w-5 h-5 ${
                      creditProposal.decision === 'approved' ? 'text-green-600' : 'text-slate-400'
                    }`} />
                    <span className={`text-sm font-medium ${
                      creditProposal.decision === 'approved' ? 'text-green-700' : 'text-slate-600'
                    }`}>Phê duyệt</span>
                  </div>
                </button>
                <button
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    <XCircle className={`w-5 h-5 ${
                      creditProposal.decision === 'rejected' ? 'text-red-600' : 'text-slate-400'
                    }`} />
                    <span className={`text-sm font-medium ${
                      creditProposal.decision === 'rejected' ? 'text-red-700' : 'text-slate-600'
                    }`}>Từ chối</span>
                  </div>
                </button>
                <button
                  ...
                  className={`p-3 rounded-lg border transition-all ${
                    creditProposal.decision === 'pending'
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    <Info className={`w-5 h-5 ${
                      creditProposal.decision === 'pending' ? 'text-amber-600' : 'text-slate-400'
                    }`} />
                    <span className={`text-sm font-medium ${
                      creditProposal.decision === 'pending' ? 'text-amber-700' : 'text-slate-600'
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  if (success) {
                    ...
                    if (creditProposal.decision === 'approved') {
                      await updateLoanStatus(loan!.id, 'pending_admin', user!.id, creditProposal.proposalNotes);
                      alert('Đã gửi đề xuất phê duyệt thành công! Hồ sơ đã chuyển sang Admin để phê duyệt cuối cùng.');
              ...
              disabled={isSubmitting}
              className={`w-full px-4 py-2.5 rounded-lg font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors ${
                creditProposal.decision === 'approved' 
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-24 bg-slate-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ 
                        width: `${Math.min(100, (loan.documents.filter(d => {
                          const key = getVerificationKey(d.type || 'other');
                          return documentVerification[key] || d.verified;
                        }).length / loan.documents.length) * 100)}%` 
                      }}
                    ></div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                {(() => {
                  ...
                  return documentTypes.map((typeInfo) => {
                    ...
                    const isCategoryVerified = documentVerification[verificationKey] || docs.every(d => d.verified);
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            ...
                            <span className={`text-xs font-medium ${
                              isCategoryVerified ? 'text-green-700' : 'text-slate-600'
                            }`}>
